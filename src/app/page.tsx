...
"use client"; // Required for GSAP animations and useEffect/useRef
...
import { ScrollTrigger } from 'gsap/dist/ScrollTrigger'; // Import ScrollTrigger if needed

gsap.registerPlugin(ScrollTrigger); // Register ScrollTrigger if you use it
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (heroRef.current) {
      gsap.fromTo(
        heroRef.current.querySelectorAll('h1, p, div > .bg-accent, div > .variant-outline'),
        { opacity: 0, y: 50 },
        {
          opacity: 1,
          y: 0,
    ...
    if (featuresRef.current) {
      gsap.fromTo(
        featuresRef.current.querySelectorAll('.bg-card'),
    ...
    if (ctaSectionRef.current) {
       gsap.fromTo(
        ctaSectionRef.current.querySelectorAll('img, h2, p, ul'),
        { opacity: 0, x: -50 },
        {
          opacity: 1,
          x: 0,
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <section ref={heroRef} className="container mx-auto flex flex-col items-center justify-center px-4 py-16 text-center md:py-24">
          ...
          <div className="mt-10 flex flex-col items-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
            <Button size="lg" asChild className="bg-accent text-accent-foreground hover:bg-accent/90 shadow-md hover:shadow-lg transition-shadow">
        ...
        <section ref={featuresRef} className="py-16 bg-secondary">
          <div className="container mx-auto px-4">
            <div className="grid md:grid-cols-3 gap-8 text-center">
              <div className="p-6 bg-card rounded-lg shadow-xl hover:shadow-2xl transition-shadow duration-300">
         ...
         <section ref={ctaSectionRef} className="container mx-auto px-4 py-16">
          <div className="flex flex-col md:flex-row items-center gap-12">
            ...
            <div className="md:w-1/2">
              <h2 className="text-3xl font-bold mb-6 text-foreground">For Job Seekers & Recruiters</h2>
              ...
              <ul className="list-disc list-inside text-muted-foreground space-y-2 text-md">
      ...
      <footer ref={footerRef} className="py-8 text-center border-t bg-secondary/50">
